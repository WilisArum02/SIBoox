...
try {
        ...
        // Query untuk mendapatkan data skripsi berdasarkan judulSkripsi
        Query skripsiQuery = entityManager.createQuery("SELECT s FROM Skripsi_1 s WHERE s.judul = :judulSkripsi");
        skripsiQuery.setParameter("judulSkripsi", judulSkripsi);

        // Menggunakan getResultList() untuk mendapatkan hasil query
        List<Skripsi_1> skripsiList = skripsiQuery.getResultList();

        // Periksa apakah hasil query ada atau tidak
        if (skripsiList.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Skripsi dengan judul " + judulSkripsi + " tidak ditemukan.");
        } else if (skripsiList.size() > 1) {
            JOptionPane.showMessageDialog(null, "Tidak dapat meminjam: Lebih dari satu skripsi dengan judul " + judulSkripsi + " ditemukan.");
        } else {
            // Hanya satu skripsi ditemukan, lanjutkan dengan proses penyimpanan
            Skripsi_1 skripsi = skripsiList.get(0);
            ...
            // Set objek Skripsi_1 ke peminjaman
            peminjamanskripsi.setIdSkripsi(skripsi);
